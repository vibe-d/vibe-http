	...
	if (isConnectionStream!ConnectionStream)
{
	...
	if (settings.decode!Base64URL(h2settings)) {
		...
		() @trusted {
			...
			if (!context.hasTable)
				context.table = FreeListRef!IndexingTable(context.settings.headerTableSize);

			// save response converted to HTTP/2
			context.resFrame = alloc.makeArray!ubyte(buildHeaderFrame!(StartLine.RESPONSE)(st, headers, context, alloc));
...
private void handleHTTP2FrameChain(ConnectionStream)(ConnectionStream stream, TCPConnection connection, HTTP2ServerContext context) @safe nothrow
	...
	static struct CB {
		...
		void opCall(bool st)
		{
			if (!st)
				...
				runTask(&handleHTTP2FrameChain, stream, connection, context);
		}
	}
...
private bool handleHTTP2Frame(ConnectionStream)(ConnectionStream stream, TCPConnection connection, HTTP2ServerContext context) @trusted
	if (isConnectionStream!ConnectionStream || is(ConnectionStream : TLSStream))
{
	...
	() @trusted {
		version (VibeManualMemoryManagement)
			scope alloc = new RegionListAllocator!(shared(Mallocator), false)(1024, Mallocator
					.instance);
		else
			scope alloc = new RegionListAllocator!(shared(GCAllocator), true)(1024, GCAllocator
					.instance);

		if (!context.hasMultiplexer)
			context.multiplexer = FreeListRef!HTTP2Multiplexer(
				alloc,
	...
	HTTP2ServerContext context, IAllocator alloc) @trusted
{
	...
	try {
		...
		logDebug("Received: " ~ to!string(
				header.type) ~ " on streamID " ~ to!string(header.streamId));

		enforceHTTP2(header.streamId % 2 != 0 || header.streamId == 0, "Clients cannot register even streams", HTTP2Error
				.PROTOCOL_ERROR);

		if (stream.needsContinuation)
			enforceHTTP2(header.type == HTTP2FrameType.CONTINUATION,
	...
	HTTP2ServerContext context, IAllocator alloc)
{
	...
	// decode headers
	decodeHPACK(cast(immutable(ubyte)[])stream.headerBlock, hdec, context.table, alloc, context
			.settings.headerTableSize);

	// insert data in table
	hdec.data.each!((h) {
		if (h.index)
			context.table.insert(h);
	});
...
void handleHTTP2SettingsFrame(Stream)(ref Stream stream, TCPConnection connection, ubyte[] data, HTTP2FrameHeader header, HTTP2ServerContext context) @safe
{
	...
	if (stream.streamId == 0)
		updateConnectionWindow(context.multiplexer, context.settings.initialWindowSize);
	updateStreamConnectionWindow(context.multiplexer, stream.streamId, context
	...
	if (isConnectionPreface(context.multiplexer))
...
struct HTTP2ConnectionStream(CS) {
	...
	@property CS connection() @safe
	{
		return m_conn;
	}

	@property HTTP2StreamState state() @safe @nogc
	{
		return m_state;
	}
	...
	@property uint streamId() @safe @nogc
	{
		return m_streamId;
	}

	@property void streamId(uint sid) @safe @nogc
	{
		m_streamId = sid;
	}

	@property HTTP2FrameStreamDependency dependency() @safe @nogc
	{
		return m_dependency;
	}
