...
ubyte[] buildHeaderFrame(alias type)(string statusLine, InetHeaderMap headers,
	HTTP2ServerContext context, ref IndexingTable table, scope IAllocator alloc, bool isTLS = true) @safe
{
	...
	// TODO padding
	if (context.next_sid == 0)
		context.next_sid = 1;

	hbuf.createHTTP2FrameHeader(cast(uint)pbuf.data.length, HTTP2FrameType.HEADERS, 0x0, context
			.next_sid);
			...
			if (type == StartLine.REQUEST) { // request
				...
				static foreach (st; __traits(allMembers, HTTPStatus)) {
					if (buf.isNumeric && __traits(getMember, HTTPStatus, st) == buf.to!int) {
						mixin(
							"H2F(\":status\",HTTPStatus." ~ st ~ ").encodeHPACK(dst, table); return;");
					}
				}
			}
		}
	...
	HTTP2HeaderTableField[] headers, ref IndexingTable table, scope IAllocator alloc) @safe
{
	...
	// initialize request
	auto req = () @trusted {
		return alloc.make!HTTPServerRequest(reqtime, listen_info.bindPort);
	}();
	scope (exit)
	...
	auto exchange = () @trusted {
		return alloc.make!(HTTP2ServerExchange!UStream)(stream,
			tcpc, h2context, headers, table, alloc);
	}();
	scope (exit)
		() @trusted { alloc.dispose(exchange); }();
	auto res = () @trusted {
		return alloc.make!HTTPServerResponse(exchange, settings, alloc);
	}();
	scope (exit)
		() @trusted { alloc.dispose(res); }();
	...
	if (req.method == HTTPMethod.HEAD)
		exchange.m_isHeadResponse = true;
	if (settings.serverString.length)
		res.headers["Server"] = settings.serverString;
	res.headers["Date"] = formatRFC822DateAlloc(reqtime);
	if (req.persistent)
		res.headers["Keep-Alive"] = formatAlloc(alloc, "timeout=%d", settings.keepAliveTimeout.total!"seconds"());

	// finished parsing the request
	parsed = true;
	logTrace("persist: %s", req.persistent);
	//keep_alive = req.persistent;
	logDebug("Received request on stream ID %d: %s %s", stream.streamId, req.method, req
			.requestPath);
...
final class HTTP2ServerExchange(UStream) : HTTPServerExchange {
	...
	void switchProtocol(HTTPServerResponse res, string protocol, scope void delegate(
			scope ConnectionStream) @safe del)
	{
		assert(false);
	}
...
@safe:
	...
	@property string data() @trusted nothrow
	{
...
void parseHTTP2RequestHeader(R)(ref R headers, HTTPServerRequest req) @safe
{
	...
	if (!host.empty)
		req.host = host[0].valueString;

	//Path
	auto pathstr = headers.find!((h, m) => h.name == m)(":path")[0].valueString;
	if (req.tls)
		req.requestURI = "https://" ~ req.host ~ pathstr;
	else
		req.requestURI = "http://" ~ req.host ~ pathstr;
